 ...
 *
 * Safety:
 *   - Refuses to run unless GBF_STRIPE_SECRET_KEY_TEST starts with "sk_test_".
 *   - Skips tiers that already have a Product with the matching name in test mode.
 *
 * Usage:
 *   GBF_STRIPE_SECRET_KEY_TEST=sk_test_... node scripts/stripe-create-products.mjs
 *
...
if (!key) {
  console.error("GBF_STRIPE_SECRET_KEY_TEST not set");
  process.exit(1);
}
if (!key.startsWith("sk_test_")) {
  console.error("Refusing to run: GBF_STRIPE_SECRET_KEY_TEST must be a TEST key (sk_test_*)");
